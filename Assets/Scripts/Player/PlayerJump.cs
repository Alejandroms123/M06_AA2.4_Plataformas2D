...
public class PlayerJump : MonoBehaviour
{
    ...
    public void HandleJump(Rigidbody2D rb, bool isWallJump, float lastMoveDir)
    {
        ...
        {
            if (_coyoteTimer > 0f && StaminaManager.instance.stamina > 0f)
            {
                dir = new Vector2(rb.linearVelocity.x, _jumpForce);
                Jump(rb, dir);
                StaminaManager.instance.LoseStamina(1f);
            }
    ...
    public void Climb(Rigidbody2D rb, float dir)
    {
        if (dir > 0f)
        {
            ...
            StaminaManager.instance.LoseStamina(Time.deltaTime * StaminaManager.instance._staminaLoseSpeed);
        }
    }
